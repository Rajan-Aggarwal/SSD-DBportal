...
#define R__DICTIONARY_FILENAME TMeasDict
...
namespace ROOT {
   ...
   static ::ROOT::TGenericClassInfo *_R__UNIQUE_DICT_(Init) = GenerateInitInstanceLocal((const ::TMeas*)0x0); R__UseDummy(_R__UNIQUE_DICT_(Init));
...
TClass *TMeas::Class()
{
   if (!fgIsA.load()) { R__LOCKGUARD(gInterpreterMutex); fgIsA = ::ROOT::GenerateInitInstanceLocal((const ::TMeas*)0x0)->GetClass(); }
   return fgIsA;
}
...
class __attribute__((annotate(R"ATTRDUMP(Edge-TCT data class)ATTRDUMP"))) __attribute__((annotate("$clingAutoload$TMeas.h")))  TMeas;
)DICTFWDDCLS";
...
nullptr};
    ...
    if (!isInitialized) {
      TROOT::RegisterModule("TMeasDict",
        ...
        TriggerDictionaryInitialization_TMeasDict_Impl, {}, classesHeaders, /*has no C++ module*/false);
      isInitialized = true;
    }
  }
